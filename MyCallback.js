...
Exemplo do tipo de função fornecida em pares de callbackDelayPairs:
...
var callback = callback(input().A, input().B)


function main(minimum, callbackDelayPairs = [...callback, delay]) {
  
  let all = [];
  let filtered = [];

    callbackDelayPairs[callback.then()].forEach(element => {
      all[element] = element;
    });

    all.forEach(element => {
      if(all[elemnt] <= minimum){
...
async function callback(A, B){
  ...
  new delay = callbackDelayPairs[deley];
  return new Promise(
    
    resolve => {

    A.register(callback, A);
